defmodule Identicon do
  ...
  Process:
    `String
    |> Computer MD5 hash of string
    |> List of #'s based on string
    |> Choose colour
  ...
  def save_image(image, filename) do
    File.write("pngs/#{filename}.png", image);
  end
  ...
  def build_grid(%Identicon.Image{hex: hex} = image) do
    grid =
      hex
      |> Enum.chunk(3)
      |> Enum.map(&mirror_row/1) # pass reference to that function
      |> List.flatten
      |> Enum.with_index
  ...
  # turns string into md5 hash, then into a list of those numbers
